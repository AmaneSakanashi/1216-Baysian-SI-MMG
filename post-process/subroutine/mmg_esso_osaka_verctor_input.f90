...
contains
    ...
    subroutine MMG_LowSpeed_model(time_derivative, state, delta_rudder, n_prop, n_bt, n_st, Wind_Direction, Wind_velocity,&
        ...
        DOUBLE PRECISION, INTENT(IN) :: pp_vector(19), mmg_params_vector(61)
        ...
        ! wind force
        AT    = pp_vector(11)  !    AT --- Transverse Projected area (m^2)
        AL    = pp_vector(12)  !    AL --- Lateral Projected area (m^2)
        AOD   = pp_vector(13) !    AOD -- Lateral Projected area of superstructure Ass and LNG tanks,
                            !           container etc. on the deck (m^2)
                            !           Here, Ass is defined as lateral Projected area of superstructure (m^2)
        LCW   = pp_vector(14) !    C ---- Distance from midship section to center of lateral projected area (m)
        LCBR  = pp_vector(15) !    CBR -- Distance from midship section to center of the Ass (m)
        HBR   = pp_vector(16)    !    HBR -- Height from free surface to top of the superstructure (bridge) (m) 
        HC    = pp_vector(17)  !    HC --- Height to center of lateral projected area (m)
        SBW   = pp_vector(18)         !    SBW -- breadth for wind area (in usual, it coincides with breadth, but it is defined for Trimaran vessel)
        Lz    = pp_vector(19)     !    Lz --- Acting position of sway force from center of gravity (it is necessaty to calculate roll moment)
        ...
        XX0 = mmg_params_vector(52)
        XX1 = mmg_params_vector(53)
        XX3 = mmg_params_vector(54)
        XX5 = mmg_params_vector(55)
        YY1 = mmg_params_vector(56)
        YY3 = mmg_params_vector(57)
        YY5 = mmg_params_vector(58)
        NN1 = mmg_params_vector(59)
        NN2 = mmg_params_vector(60)
        NN3 = mmg_params_vector(61)
        ! KK1 = 0.0d0
        ! KK2 = 0.0d0
        ! KK3 = 0.0d0
        ! KK5 = 0.0d0
